...
install_zimage() {
    ...
    if [ "$DRY_RUN" = false ]; then
        sudo -u comfyui /home/comfyui/.local/bin/uv --no-config pip install "accelerate>=1.2.0"
        # Ensure VAE decoding doesn't hang (Triton/FlashAttention)
        sudo -u comfyui /home/comfyui/.local/bin/uv --no-config pip install "triton>=3.0.0"
    fi
}
