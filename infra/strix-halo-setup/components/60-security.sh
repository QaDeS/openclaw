...
install_cisco_defense() {
    ...
    if [ "$DRY_RUN" = false ]; then
        if [ ! -f /home/defense/.local/bin/uv ]; then
...
install_hosting_stack() {
    ...
    run mkdir -p "$quadlet_dir" "$systemd_dir"
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        # Deploy grouping target (plain systemd unit, not a quadlet)
        cp "${INFRA_DIR}/quadlet/hosting.target" "${systemd_dir}/hosting.target"
        track_file_create "${systemd_dir}/hosting.target"

        chown -R hosting:hosting /home/hosting/.config
        ensure_linger hosting
        user_systemctl hosting daemon-reload
        user_systemctl hosting enable hosting.target
        user_systemctl hosting start hosting.target
    else
        run echo "deploy quadlet files for hosting stack"
        run echo "enable linger + start hosting.target for hosting user"
    fi

    track_service "hosting (user@hosting)"
}
