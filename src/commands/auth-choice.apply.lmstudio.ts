...
export async function applyAuthChoiceLmStudio(
    ...
    const mode = await params.prompter.select({
        ...
        options: [
            { value: "api", label: "Local Server API", hint: "http://localhost:1234 (default)" },
            { value: "folder", label: "Model Folder", hint: "Directly scan .gguf files" },
        ],
    });
    ...
    if (mode === "folder") {
        ...
        const url = await params.prompter.text({
            ...
            validate: (value) => (value?.trim().startsWith("http") ? undefined : "Must be a valid HTTP URL"),
        });
