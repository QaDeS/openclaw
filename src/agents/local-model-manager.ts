...
export class LocalModelManager {
  ...
  async getModel(modelPath: string): Promise<LlamaModel> {
    ...
    if (existing) {
      log.info(`[LocalModelManager] Reusing loaded model: ${modelPath}`);
    ...
    log.info(`[LocalModelManager] Loading model: ${modelPath}`);
    try {
      ...
      log.error(`[LocalModelManager] Failed to load model ${modelPath}:`, err as any);
      throw err;
    }
  }
  ...
  private async ensureLlama() {
    ...
    log.info("[LocalModelManager] Initializing node-llama-cpp runtime...");
  ...
  async unloadModel(modelPath: string) {
    ...
    if (entry) {
      log.info(`[LocalModelManager] Explicitly unloading model: ${modelPath}`);
  ...
  async clearCache() {
    log.info("[LocalModelManager] Clearing all cached models");
