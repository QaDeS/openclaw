...
export async function resolveImplicitLocalProvider(params: {
  ...
  if (apiUrl) {
    ...
    return {
      baseUrl: finalUrl,
      apiKey: providerConfig?.apiKey || params.env?.LOCAL_LLM_TOKEN || "none",
      api: (providerConfig?.api || params.env?.LOCAL_LLM_API || "openai-responses") as any,
      models: [], // Discovery happens via API probing in discovery source
    };
  }

  // 2. File Mode Check (file:// URL for local model discovery)
